...
pub fn setup_firewall_state(
    ...
    change_default_zone_button: gtk::Button,
    status_page: adw::StatusPage,
    interfaces_listbox: gtk::ListBox,
) {
    let client_for_btn = client.clone();
    let status_page_clone = status_page.clone();
    let toast_overlay_clone = toast_overlay.clone();

    change_default_zone_button.connect_clicked(move |btn| {
        show_change_default_zone_dialog(
            client_for_btn.clone(),
            status_page_clone.clone(),
            btn.clone(),
...
pub fn show_change_default_zone_dialog(
    client: FirewallClient,
    status_page: adw::StatusPage,
    ...
    glib::spawn_future_local(async move {
        let zones = client.fetch_zones().await.unwrap_or_default();
        let current_default = client.fetch_default_zone().await.unwrap_or_default();

        let dialog = adw::AlertDialog::builder()
            .heading("Change Default Fallback Zone")
            .build();

        let zone_strs: Vec<&str> = zones.iter().map(|s| s.as_str()).collect();
        let combo = gtk::DropDown::from_strings(&zone_strs);

        if let Some(pos) = zones.iter().position(|z| z == &current_default) {
            combo.set_selected(u32::try_from(pos).unwrap_or(0));
        }
        ...
        let label = gtk::Label::builder()
            .label("Select a new default zone for the system:")
            .halign(gtk::Align::Start)
            .margin_bottom(8)
            .build();
        ...
        let response = dialog.choose_future(&parent_btn).await;
        if response == "save" {
            let selected_idx = combo.selected() as usize;
            if let Some(new_zone) = zones.get(selected_idx) {
                match client.set_default_zone(new_zone).await {
                    Ok(_) => {
                        status_page.set_description(Some(&format!("Fallback Zone: {}", new_zone)));
                        show_toast(&toast_overlay, "Default zone updated successfully.");
                    }
                    Err(e) => show_toast(&toast_overlay, &format!("Failed to change default zone: {}", e)),
                }
            }
        }
    });
